...
def permute_voters_mlp(X_batch, cand_max, vot_max) -> torch.Tensor:
    ...
    # generate a random voter permutation on same device
    perm = torch.randperm(vot_max, device=device)
...
Profile
object(
with .rankings as list of voter rankings)
winners: torch.Tensor
of
shape(cand_max, ), 1
for winners and 0 otherwise
cand_max: int, number
of
candidates
winner_method: _anonymity(profile, winners, cand_max, winner_method="borda", n_permutations=50):
"""Check whether str, voting rule ("borda", "plurality", "copeland")
...
# Move to CPU for profile operations
device = winners.device
original_winners = winners.clone().int().cpu()

# Get voter list safely
voter_rankings = list(profile.rankings)
...
for perm in perms:
    ...
    permuted_winners = torch.zeros(cand_max, dtype=torch.int, device='cpu')
...
def permute_candidates_mlp(X_batch, max_cand, max_vot, perm) -> torch.Tensor:
   ...
   """
    ...
    device = X_batch.device
    X_perm = torch.zeros_like(X_batch, device=device)

    for i in range(batch_size):
        profile = X_batch[i].view(max_vot, max_cand, max_cand)
        permuted_profile = profile[:, perm][:, :, perm]
...
def permute_candidates_wec(profiles, cand_max, vot_max, perm) -> list[pref_voting.profiles.Profile]:
    ...
    # Move perm to CPU for profile operations
    perm_cpu = perm.cpu() if isinstance(perm, torch.Tensor) else perm

    for prof in profiles:
        candidates = list(prof.candidates)
        n = len(candidates)

        permutation_dict = {}
        for i, cand in enumerate(candidates):
            if i < len(perm_cpu):
                target_idx = perm_cpu[i].item() % n if isinstance(perm_cpu, torch.Tensor) else perm_cpu[i] % n
...
def neutrality_loss(model, X_batch, max_cand, max_vot, num_samples=50) -> torch.Tensor:
    ...
    for _ in range(num_samples):
        perm = torch.randperm(max_cand, device=device)
...
def check_neutrality(profile, winners, cand_max, winner_method="borda") -> int:
    ...
    """
    original_winners = winners.cpu() if winners.is_cuda else winners
...
def independence_loss(model, X_batch, prof, max_cand, max_vot, num_samples=50) -> torch.Tensor:
    ...
    device = original_prediction.device
    ...
    for _ in range(num_samples):
        ...
        for p in X_nontrivial_p:
            ...
            if model.name == "mlp":
                permuted_prof_enc = utils.encode_mlp(Profile(permuted_prof), max_cand, max_vot).to(device)
                X_permuted.append(permuted_prof_enc)
            elif model.name == "cnn":
                permuted_prof_enc = utils.encode_cnn(Profile(permuted_prof), max_cand, max_vot).to(device)
                X_permuted.append(permuted_prof_enc)
            elif model.name == "wec":
...
def check_independence(profile, winners, cand_max, winner_method="borda", n_permutations=None) -> int:
    ...
    winners_cpu = winners.cpu() if winners.is_cuda else winners
    winners_list = torch.nonzero(winners_cpu, as_tuple=True)[0].tolist()[:profile.num_cands]
    losers = [i for i in range(profile.num_cands) if i not in winners_list]

    satisfaction = 0

    if winners_list == set(profile.candidates) or winners_list == set([]):
        satisfaction = 0
        return satisfaction
    else:
