"""VotingMLPPerVoter: Per-voter pairwise MLP for voting classification."""
...
from principle_vote.axioms import (
    set_training_axiom,
    check_anonymity,
    check_neutrality,
    check_condorcet,
...
class VotingMLPPerVoter(nn.Module):
    """MLP using per-voter pairwise matrices as input."""

    def __init__(self, train_loader: DataLoader, max_candidates: int, max_voters: int):
        super().__init__()
        self.name = "mlp_per_voter"
        self.encoding_type = "pairwise_per_voter"
        self.max_cand = max_candidates
        self.max_vot = max_voters

        tri_size = max_candidates * (max_candidates - 1) // 2
        self.input_size = tri_size * max_voters

        # Hidden widths chosen to keep total params close (~+3%) to baseline pairwise network.
        ...
        for idx, out_dim in enumerate(hidden_dims):
            ...
            in_dim = out_dim
        layers.append(nn.Linear(in_dim, self.max_cand))
    ...
    def evaluate_model_hard(self, X_test: torch.Tensor, y_test: torch.Tensor) -> float:
        ...
        with torch.no_grad():
            ...
            print(f"Hard Accuracy: {accuracy}")
            return accuracy
    ...
    def evaluate_model_soft(self, X_test: torch.Tensor, y_test: torch.Tensor) -> float:
        ...
        with torch.no_grad():
            ...
            accuracy = correct / len(y_test)
            print(f"Soft Accuracy: {accuracy}")
            return accuracy
    ...
    def evaluate_axiom_satisfaction(self, data: SynthData, axiom: str) -> float:
        ...
        rate = satisfied / len(y_test)
        print(f"Axiom ({axiom}) Satisfaction Rate: {rate}")
        return rate
    ...
    def plot_training_loss(self, steps: List[int], losses: List[float]):
        ...
        plt.ylabel('Loss')
        plt.title('Per-voter MLP Training Loss')
        plt.legend()
